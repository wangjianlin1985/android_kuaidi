...
public class TakeOrderEditActivity extends Activity {
	// 声明确定添加按钮
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		// 设置当前Activity界面布局
		setContentView(R.layout.takeorder_edit); 
		ImageView search = (ImageView) this.findViewById(R.id.search);
		search.setVisibility(View.GONE);
		TextView title = (TextView) this.findViewById(R.id.title);
		title.setText("编辑代拿订单信息");
			...
			public void onClick(View v) {
				try {
					...
					if(ET_takeTime.getText().toString().equals("")) {
						Toast.makeText(TakeOrderEditActivity.this, "接单时间输入不能为空!", Toast.LENGTH_LONG).show();
					...
					if(ET_ssdt.getText().toString().equals("")) {
						Toast.makeText(TakeOrderEditActivity.this, "实时动态输入不能为空!", Toast.LENGTH_LONG).show();
					...
					if(ET_evaluate.getText().toString().equals("")) {
						Toast.makeText(TakeOrderEditActivity.this, "用户评价输入不能为空!", Toast.LENGTH_LONG).show();
					...
					/*调用业务逻辑层上传代拿订单信息*/
					TakeOrderEditActivity.this.setTitle("正在更新代拿订单信息，稍等...");
	...
	private void initViewData() {
		...
		for (int i = 0; i < expressTakeList.size(); i++) {
			if (takeOrder.getExpressTakeObj() == expressTakeList.get(i).getOrderId()) {
				this.spinner_expressTakeObj.setSelection(i);
				break;
			}
		}
		for (int i = 0; i < userInfoList.size(); i++) {
			if (takeOrder.getUserObj().equals(userInfoList.get(i).getUser_name())) {
				this.spinner_userObj.setSelection(i);
				break;
			}
		}
		this.ET_takeTime.setText(takeOrder.getTakeTime());
		for (int i = 0; i < orderStateList.size(); i++) {
			if (takeOrder.getOrderStateObj() == orderStateList.get(i).getOrderStateId()) {
				this.spinner_orderStateObj.setSelection(i);
				break;
			}
		}
		this.ET_ssdt.setText(takeOrder.getSsdt());
		this.ET_evaluate.setText(takeOrder.getEvaluate());
	}
	@Override
